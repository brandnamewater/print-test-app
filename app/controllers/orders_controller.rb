class OrdersController < ShopifyApp::AuthenticatedController
  ...
  def index
    ...
    store_products = ShopifyAPI::Order.where(:params => { :line_items => { :product_id => @store_product_ids } } )

    @store_products = store_products.find(:params => { :line_items => { :product_id => @store_product_ids } } )
